...
available tools (these are actually functions with well defined inputs and outputs. Strictly keep only following inputs, outputs for them) :- 
...
6. semantic_search : search similar text using embeddinngs. input : search_query, output : top_k_results (list)
...
8. code_runner : run the code and give the output. inputs : code_language, code generated by coder agent (llm); output : output_logs
...
def process_subtask(agent_list,sub_task,notebook,prev_sub_task):
    global code
    sub_note={}
    idx=sub_task["id"]
...
def create_agents(question:str):
    ...
    for sub_task in enhanced_prompt["sub_tasks"]:
        ...
        if enhanced_prompt['inputs']:
            ...
            agent_list=rational_plan_node(sub_task['subtask_description'])
        process_subtask(agent_list,sub_task,notebook,opt_frm_prev_sub_task)
        st.write("-" * 20)
        st.write(f"SUBTASK {sub_task['id']} COMPLETED. FULL NOTEBOOK OUTPUTS:-")
        # Display all files in TEMP/NEW directory
        new_files = glob.glob("TEMP/NEW/*")
        # st.write("Files in TEMP/NEW directory:")
        for file in new_files:
            if file.endswith(('.png', '.jpg', '.jpeg', '.gif', '.bmp')):
                st.image(file, caption=f"Newly created or modified image: {os.path.basename(file)}", use_column_width=True)
            st.write(file)

        # Delete all files in TEMP/NEW directory
        # for file in new_files:
        #     os.remove(file)
